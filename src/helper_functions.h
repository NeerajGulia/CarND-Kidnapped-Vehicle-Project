...
inline double calculateMultivariateGaussian(double sig_x, double sig_y, double x, double y, double mu_x, double mu_y)
{
	return exp( -( pow((x - mu_x), 2) / 2 * pow(sig_x, 2) + pow( (y - mu_y), 2)/2*pow(sig_y, 2) )  ) / 2.0 * M_PI * sig_x * sig_y;
}
